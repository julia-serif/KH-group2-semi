...
<body>
	...
	<div align="center">
		...
		<c:if test="${!empty dto }">
					...
					<td>
						<form method="post" name="frm">
							...
							<table border="0" cellspacing="0" width="85%" align="center">
								<tr>
									<td align="left" style="padding-left: 3px;"><input
										type="number" name="p_qty" min="1" max="50" value="1">
									</td>
								</tr>

								<tr>
									<td align="left"><a href="javascript:goCart()"> <img
											src="<%=request.getContextPath()%>/uploadfile/1.PNG"
											border="0">
									</a></td>

									<td align="left"><a href="javascript:goOrder()"> <img
											src="<%=request.getContextPath()%>/uploadfile/2.PNG"
											border="0">
									</a></td>
								</tr>
							</table>
			...
			<table border="1" cellspacing="0" width="650" id="bbs_list">
				...
				<c:if test="${!empty list }">
					<c:forEach items="${list }" var="dto">
						<tr align="center">
							<td>
								<%-- 답변인 경우 --%> <c:if test="${dto.getBoard_indent() != 0 }">
									<c:forEach begin="1" end="${dto.getBoard_indent() }">
									↳
								</c:forEach>
								</c:if> <a
								href="<%=request.getContextPath() %>/bbs_content.do?no=${dto.getBoard_no() }">
				...
				<tr id="qna">
					<td colspan="3" align="right"><input type="button"
						value="문의하기" onclick="location.href='bbs_write.do?pno=${pno }'">
					</td>
				</tr>
			...
			<ul class="qna">
				<li><input type="checkbox" id="qna_1"> <label
					for="qna_1">상품 취소/반품</label>
					<div>
						<p>구매한 상품의 취소/반품은 마이페이지 구매내역에서 신청</p>
						<p>가능합니다. 상품문의 및 후기게시판을 통해 취소나 환불,</p>
						<p>반품 등은 처리되지 않습니다.</p>
					</div></li>
				<li><input type="checkbox" id="qna_2"> <label
					for="qna_2">교환/환불 및 배송등 상품 자체와 관련 없는 문의</label>
					<div>
						<p>가격, 판매자, 교환/환불 및 배송 등 해당 상품 자체와 관련</p>
						<p>없는 문의는 고객센터 내 1:1 문의하기를 이용해주세요.</p>
					</div></li>
				<li><input type="checkbox" id="qna_3"> <label
					for="qna_3">배송 소요일</label>
					<div>
